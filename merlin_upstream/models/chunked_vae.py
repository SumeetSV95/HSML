...
class CHUNKED_VAE(nn.Module):
    def __init__(self, num_chunks):
        ...
        self.chunk_embeddings = nn.Parameter(torch.empty(num_chunks, latent_dim), requires_grad=True)
        nn.init.normal_(self.chunk_embeddings, mean=0.0, std=0.02)

    def encoder(self, x):
        act = torch.relu(self.do1(self.e1(x)))
        mean = self.mu(act)
        log_variance = torch.clamp(self.log_var(act), min=-10.0, max=10.0)
        return mean, log_variance

    def reparameterize(self, mean, log_var):
        sd = torch.exp(0.5 * torch.clamp(log_var, min=-10.0, max=10.0))
    ...
    def forward(self, x, c, chunk_id):
        ...
        l_v_prior = torch.clamp(self.log_var_prior(c), min=-10.0, max=10.0)

...
class _KMeansCompat:
    """
    Minimal drop-in shim for tf.contrib.learn.KMeansClustering used in HSML.
    Supports:
      - ctor: _KMeansCompat(num_clusters=K, **kwargs)
      - train(input_fn, steps=None)    -> fits on input_fn() data
      - predict_cluster_index(input_fn)-> returns cluster indices for input_fn() data
      - cluster_centers_               -> numpy array of centers
    Assumes input_fn returns either:
      - a numpy array of shape [N, D], or
      - a tuple (X, y) where X is [N, D] and y is ignored.
    """
    def __init__(self, num_clusters, **kwargs):
        # Map TF args roughly; ignore others
        self.k = int(num_clusters)
        # n_init='auto' requires sklearn>=1.4; fall back to 10 for older versions
        try:
            self._kmeans = KMeans(n_clusters=self.k, n_init='auto', random_state=0)
        except TypeError:
            self._kmeans = KMeans(n_clusters=self.k, n_init=10, random_state=0)

    def _get_X(self, input_fn):
        if input_fn is None:
            raise ValueError("input_fn must be provided")
        data = input_fn()
    ...
    def train(self, input_fn=None, steps=None):
        X = self._get_X(input_fn)
        self._kmeans.fit(X)
        return self

    def predict_cluster_index(self, input_fn=None):
        X = self._get_X(input_fn)
        return list(self._kmeans.predict(X))

    @property
    def cluster_centers_(self):
        return self._kmeans.cluster_centers_

# Alias the old name so the rest of the code keeps working
KMeansClustering = _KMeansCompat
...
class TreeLSTM(object):
    ...
    def model(self, inputs):
        ...
        if FLAGS.datasource in ['multidataset', 'miniimagenet', 'mnist_rotations', 'mnist_permutations', 'nonstationary']:
            sigma=10.0

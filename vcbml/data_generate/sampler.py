...
class SuppQueryBatchSampler(Sampler):
    def __init__(self, dataset, subset_indices_per_cls=None, seqtask=True, num_batch=10, num_task=100,
        ...
        if subset_indices_per_cls is None and hasattr(dataset, 'subset_indices_per_cls'):
            subset_indices_per_cls = dataset.subset_indices_per_cls
        self.subset_indices_per_cls = subset_indices_per_cls
        self.seqtask = seqtask
        if self.seqtask and hasattr(dataset, 'seqtask_num_batch'):
            self.num_batch = dataset.seqtask_num_batch
        else:

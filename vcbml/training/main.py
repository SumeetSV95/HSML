...
from data_generate.rotated_mnist_dataset import RotatedMNISTDataset
...
def train(config, run_spec, seed=0):
    ...
    for task_idx, task in enumerate(config['dataset_ls'][:num_dataset_to_run], 0):
        if config['completed_task_idx'] is not None and config['completed_task_idx'] >= task_idx:
            ...
            if task == 'rotated_mnist':
                trainset = RotatedMNISTDataset(
                    pt_file=config[task]['pt_file'], split='train', transform=transformation,
                    device=config['device'], cuda_img_tensor=config['cuda_img_tensor'],
                    digits=config[task].get('digits'), num_way=config['net_kwargs']['num_way'],
                    num_shot=config[task]['num_shot'], num_query=config[task]['num_query_per_cls'],
                    verbose='{} trainset'.format(task)
                )

                evalset.append(RotatedMNISTDataset(
                    pt_file=config[task]['pt_file'], split='test', transform=transformation,
                    device=config['device'], cuda_img_tensor=config['cuda_img_tensor'],
                    digits=config[task].get('digits'), num_way=config['net_kwargs']['num_way'],
                    num_shot=config[task]['num_shot'], num_query=config[task]['num_query_per_cls'],
                ...
                current_seqtask = getattr(trainset, 'seqtask', config['seqtask'])
            else:
                ...
                current_seqtask = config['seqtask']
            ...
            metatrain_seqdataset_amvi(
                ...
                lr_inner=config[task]['lr_inner'], first_order=config[task]['first_order'], seqtask=current_seqtask,
